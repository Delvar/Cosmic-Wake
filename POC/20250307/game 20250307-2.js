class Camera {
    ...
    worldPositionToScreen(worldPos) {
        ...
        return new Vector2D(screenCenter.x + dx, screenCenter.y + dy);
    }
    ...
    worldPositionToCamera(worldPos) {
        const dx = (worldPos.x - this.position.x) * this.zoom;
        const dy = (worldPos.y - this.position.y) * this.zoom;
        return new Vector2D(dx, dy);
    }

    cameraPositionToScreen(cameraPos) {
        const screenCenter = this.getScreenCenter();
        return new Vector2D(screenCenter.x + cameraPos.x, screenCenter.y + cameraPos.y);
    }
}
...
class StarField {
    ...
    draw(context, camera, shipVelocity) {
        ...
        const centerScreen = camera.getScreenCenter();
...
class CelestialBody {
    ...
    draw(context, camera) {
        context.save();
        const screenPos = camera.worldPositionToScreen(this.position);
...
class HeadsUpDisplay {
    constructor(ship, celestialBodies, canvasWidth, canvasHeight) {
        ...
        this.gateRingRadius = Math.min(canvasWidth, canvasHeight) / 2.5;
    }
    ...
    draw(context, camera) {
        const screenCenter = camera.getScreenCenter();
        context.save();
        // Inner ring (planets, etc.)
        context.beginPath();
        context.arc(screenCenter.x, screenCenter.y, this.ringRadius, 0, Math.PI * 2);
        ...
        context.beginPath();
        context.arc(screenCenter.x, screenCenter.y, this.gateRingRadius, 0, Math.PI * 2);
        context.strokeStyle = 'rgba(0, 255, 0, 0.3)';
        context.lineWidth = 2;
        context.stroke();
        context.closePath();

        const remapClampRange = (numberIn, rangeInMin, rangeInMax, rangeOutMin, rangeOutMax) => {
            return Math.min(Math.max((rangeOutMax - rangeOutMin) * (numberIn - rangeInMin) / (rangeInMax - rangeInMin) + rangeOutMin, rangeOutMin), rangeOutMax);
        };

        this.celestialBodies.forEach(body => {
            const bodyCameraPosition = camera.worldPositionToCamera(body.position);
            const bodyCameraCenterDistance = Math.sqrt(bodyCameraPosition.x * bodyCameraPosition.x + bodyCameraPosition.y * bodyCameraPosition.y);
            const bodyScreenPosition = camera.worldPositionToScreen(body.position);
            // Draw name if within 500 units
            if (bodyCameraCenterDistance < this.ringRadius && body.name) {
                context.save();
                ...
                context.fillText(body.name, bodyScreenPosition.x, bodyScreenPosition.y + scaledRadius + camera.worldSizeToScreen(20));
                context.restore();
            }

            // Draw HUD arrows if outside the ring
            const isGate = body instanceof JumpGate;
            const radius = isGate ? this.gateRingRadius : this.ringRadius;
            if (bodyCameraCenterDistance > radius) {
                const angle = Math.atan2(bodyCameraPosition.y, bodyCameraPosition.x);
                const arrowScreenPositionX = screenCenter.x + Math.cos(angle) * radius;
                const arrowScreenPositionY = screenCenter.y + Math.sin(angle) * radius;
                const color = body.subtype ? body.subtype.color : body.type.color;
                const bodyRingDistance = bodyCameraCenterDistance - radius;
                const opacity = remapClampRange(bodyRingDistance, 5000, 0, 0.2, 1);
                context.fillStyle = color.toRGB();
                context.globalAlpha = opacity;
                context.save();
                context.translate(arrowScreenPositionX, arrowScreenPositionY);
...
class Game {
    constructor() {
        ...
        const earth = this.currentSystem.celestialBodies[3];
        this.playerShip = new Ship(earth.position.x + 50, earth.position.y); // Start near Sol's Earth
