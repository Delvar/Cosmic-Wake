...
class AIShip extends Ship {
    constructor(system) {
        ...
        this.state = 'flying'; // 'flying' or 'landing'
        console.log('AIShip Take Off: ', this);
    }
    ...
    update(dt, system) {
        ...
        // Thrust or brake
        const speedSquared = this.velocity.x * this.velocity.x + this.velocity.y * this.velocity.y;
        if (distSquared > 2500) { // > 50 units, thrust
            this.state = 'flying';
            const thrustX = Math.cos(this.angle) * this.thrust * dt;
            const thrustY = Math.sin(this.angle) * this.thrust * dt;
            this.velocity.x += thrustX;
            this.velocity.y += thrustY;
        } else if (speedSquared > 0.01) { // < 50 units, slow to < 0.1
            this.state = 'landing';
            const drag = this.brakeDrag;
            this.velocity.x -= this.velocity.x * drag * dt;
            this.velocity.y -= this.velocity.y * drag * dt;
            const velAngle = Math.atan2(-this.velocity.y, -this.velocity.x);
            const velAngleDiff = (velAngle - this.angle + Math.PI) % (2 * Math.PI) - Math.PI;
            this.angle += Math.min(Math.max(velAngleDiff, -this.rotationSpeed * dt), this.rotationSpeed * dt);
        } else { // Landed, despawn
            const index = system.ships.indexOf(this);
            if (index > -1) system.ships.splice(index, 1);
            console.log('AIShip Land: ', this);
            return;
        }
...
class Game {
    ...
    update(deltaTime) {
        ...
        this.currentSystem.ships.forEach(ship => {
            if (ship instanceof AIShip) ship.update(deltaTime, this.currentSystem);
        });
