...
class AIShip extends Ship {
    constructor(system) {
        ...
        super(spawnPlanet.pos.x + Math.cos(angle) * 50, spawnPlanet.pos.y + Math.sin(angle) * 50);
        this.spawnPlanet = spawnPlanet;
        this.targetPlanet = this.pickTargetPlanet(system);
        this.state = 'flying';
    }

    pickTargetPlanet(system) {
        const planets = system.bodies.filter(body => body !== this.spawnPlanet && body.type.type !== 'star' && !(body instanceof JumpGate));
        return planets.length > 0 ? planets[Math.floor(Math.random() * planets.length)] : this.spawnPlanet;
    }

    update(dt, system) {
        dt = dt / 16.67;
        const dx = this.targetPlanet.pos.x - this.pos.x;
        const dy = this.targetPlanet.pos.y - this.pos.y;
        const distSquared = dx * dx + dy * dy;
        const speedSquared = this.velocity.x * this.velocity.x + this.velocity.y * this.velocity.y;
        const speed = Math.sqrt(speedSquared);

        // Calculate stopping distance: v² / (2 * a), where a = brakeDrag * thrust
        const stopDist = speedSquared / (2 * this.brakeDrag * this.thrust);
        const stopDistSquared = stopDist * stopDist;

        if (distSquared > 2500 && (distSquared > stopDistSquared || speedSquared < 1)) { // > 50 units, flying
            this.state = 'flying';
            ...
            this.velocity.x += thrustX;
            this.velocity.y += thrustY;
        } else if (speedSquared > 0.01) { // < 50 units or stopping distance, brake
            this.state = 'landing';
            const brakeAngle = Math.atan2(-dy, -dx); // Face away from planet
            const angleDiff = (brakeAngle - this.angle + Math.PI) % (2 * Math.PI) - Math.PI;
            this.angle += Math.min(Math.max(angleDiff, -this.rotationSpeed * dt), this.rotationSpeed * dt);
            this.angle = ((this.angle + Math.PI) % (2 * Math.PI)) - Math.PI;
            const thrustX = Math.cos(this.angle) * this.thrust * dt; // Thrust away
            const thrustY = Math.sin(this.angle) * this.thrust * dt;
            this.velocity.x += thrustX;
            this.velocity.y += thrustY;
        } else if (distSquared < 2500) { // < 50 units and slow, land
            const index = system.ships.indexOf(this);
            if (index > -1) system.ships.splice(index, 1);
            return;
        }

        // Apply drag and velocity
        const drag = this.state === 'flying' ? this.drag : this.brakeDrag;
        this.velocity.x -= this.velocity.x * drag * dt;
        this.velocity.y -= this.velocity.y * drag * dt;
        if (speedSquared > this.maxVelocity * this.maxVelocity) {
    ...
    draw(ctx, cam) {
        ...
        ctx.fillStyle = 'gray'; // Gray to distinguish from player (white)
        ctx.beginPath();
        const scale = cam.zoom;
...
class Game {
    constructor() {
        ...
        this.playerShip = new Ship(earth.pos.x + 50, earth.pos.y);
    ...
    randomSpawnInterval() {
        return 2000 + Math.random() * 8000; // 2–10 seconds
    }
    ...
    update(dt) {
        ...
        this.currentSystem.ships.forEach(ship => {
            if (ship instanceof AIShip) ship.update(dt, this.currentSystem);
        });
        ...
        // Random AI ship spawning
        if (currentTime - this.lastSpawnTime > this.spawnInterval && this.currentSystem.ships.length < 10) {
            this.currentSystem.ships.push(new AIShip(this.currentSystem));
    ...
    render(dt) {
        ...
        this.currentSystem.bodies.forEach(body => body.draw(this.ctx, this.cam));
        this.currentSystem.ships.forEach(ship => ship.draw(this.ctx, this.cam));
