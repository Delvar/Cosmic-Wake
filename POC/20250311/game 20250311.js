...
class AIShip extends Ship {
    ...
    update(dt, system, game) {
        ...
        //instead of respawning we simply find a new target!
        ...
        if (distance > 0) {
            ...
            relativeDirection = new Vector2D(1, 0); // Default if at target
        }
        if (speed > 0) {
            velocityDirection = new Vector2D(this.velocity.x / speed, this.velocity.y / speed);
        } else {
            velocityDirection = relativeDirection; // No velocity, use target direction
        }
        ...
        // Unwanted velocity (perpendicular)
        ...
        let thrusting = false;
        if (this.state == 'landing') {
            //check if we are going to overshoot and need to thrust back, tighter angle dif on landing thrust as we have a small target to aim for
            if ((stopDistance > distance + this.targetPlanet.radius) && (Math.abs(angleDiff) < Math.PI * 0.01)) {
                thrusting = true;
            } else {
                //we must let it coast 
                thrusting = false;
            }
        ...
        // only thrust if looking towards the thrust angle
        if (thrusting) {
            this.angle = ((this.angle + Math.PI) % (2 * Math.PI)) - Math.PI;
            const thrustX = Math.cos(this.angle) * this.thrust * dt;
            const thrustY = Math.sin(this.angle) * this.thrust * dt;
...
class Game {
    constructor() {
        ...
        this.camTarget = this.playerShip; // Start on player
        ...
        window.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'Tab') this.cycleNextAIShip();
    ...
    cycleNextAIShip() {
        const aiShips = this.currentSystem.ships.filter(ship => ship instanceof AIShip);
    ...
    update(dt) {
        ...
        this.currentSystem.ships.forEach(ship => {
            if (ship instanceof AIShip) ship.update(dt, this.currentSystem, this); // Pass this (Game)
        });
        this.cam.update(this.camTarget.pos); // Use camTarget
    ...
    tryHyperjump() {
        ...
        if (gate && this.playerShip.initiateHyperjump(gate.lane.target, currentTime)) {
            ...
            this.camTarget = this.playerShip; // Reset to player on jump
