...
class HeadsUpDisplay {
    ...
    draw(ctx, cam) {
        ...
        // Ships (updated radius for ship ring)
...
class PlayerPilot extends Pilot {
    ...
    update(dt, game) {
        dt = dt / 16.67; // Normalize to ~60 FPS
...
class AIPilot extends Pilot {
    ...
    update(dt, game) {
        ...
        console.log(`AI Ship: System=${this.ship.system.name}, State=${this.state}, Distance=${distance.toFixed(2)}, Speed=${speed.toFixed(2)}`);
        ...
        if (distance < this.targetPlanet.radius && speed < 0.5) {
            ...
            if (this.targetPlanet instanceof JumpGate) {
                ...
                if (this.ship.initiateHyperjump(this.targetPlanet.lane.target, currentTime)) {
                    ...
                    console.log(`AI Ship jumped to ${this.ship.system.name}, Old System Ships: ${oldSystem.ships.length}, New System Ships: ${this.ship.system.ships.length}`);
                ...
                if (excess > 0) {
                    ...
                    if (Math.random() < despawnChance) {
                        console.log(`Despawned in ${this.ship.system.name} (excess: ${excess})`);
...
class Game {
    constructor() {
        ...
        this.playerShip.pilot = this.playerPilot; // Explicitly set pilot
    ...
    update(dt) {
        ...
        // Update all pilots and ships uniformly with safety check
        this.galaxy.forEach(system => {
            system.ships.forEach(ship => {
                if (ship.pilot) { // Only update if pilot exists
                    ship.pilot.update(dt, this);
                } else {
                    console.warn("Ship missing pilot:", ship);
                }
                ship.update(dt);
            });
        });
