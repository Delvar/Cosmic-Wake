...
class HeadsUpDisplay {
    ...
    draw(ctx, cam) {
        ...
        if (this.game.camTarget instanceof Ship) {
        ...
        if (target) {
            ...
            // Only draw arrow if target is outside its ring
            if (distSquared > ringRadius * ringRadius) {
                const angle = Math.atan2(camPos.y, camPos.x);
                const arrowX = center.x + Math.cos(angle) * ringRadius; // Base on ring
...
class AIPilot extends Pilot {
    ...
    update(dt, game) {
        ...
        this.ship.setTarget(this.targetPlanet); // Sync target with targetPlanet
        ...
        if (distance < this.targetPlanet.radius && speed < 0.5) {
            ...
            if (this.targetPlanet instanceof JumpGate) {
                ...
                if (this.ship.initiateHyperjump(this.targetPlanet.lane.target, currentTime)) {
                    ...
                    this.ship.setTarget(this.targetPlanet); // Update target after jump
                    return;
                }
            } else {
                ...
                if (excess > 0) {
                    ...
                    if (Math.random() < despawnChance) {
                        ...
                        this.ship.clearTarget(); // Clear target on despawn
                        return;
                    }
                }
                this.targetPlanet = this.pickDestination(this.ship.system, this.spawnPlanet);
                this.ship.setTarget(this.targetPlanet); // Update target on reroute
            }
            return;
        }
...
class Game {
    ...
    getTargetableObjects() {
        ...
        const planets = system.bodies.filter(body => !(body instanceof JumpGate));
        const gates = system.bodies.filter(body => body instanceof JumpGate);
        const ships = system.ships.filter(ship => ship !== this.playerShip); // Exclude player ship
        const asteroids = system.asteroidBelt ? system.asteroidBelt.interactiveAsteroids : [];
    ...
    cyclePreviousTarget() {
        ...
        const currentIndex = targets.indexOf(this.playerShip.target);
        const prevIndex = (currentIndex - 1 + targets.length) % targets.length; // Wrap around
