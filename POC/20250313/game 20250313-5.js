...
class JumpGate extends GameObject {
    constructor(lane, sysPos) {
        const dir = new Vector2D(lane.target.position.x - sysPos.x, lane.target.position.y - sysPos.y);
...
class Game {
    ...
    update(deltaTime) {
        ...
        this.camera.update(this.cameraTarget.position);
        if (this.zoomTextTimer > 0) {
            this.zoomTextTimer -= deltaTime;
        }
        const currentTime = performance.now();
        ...
        if (this.cameraTarget instanceof Ship && this.cameraTarget.target) {
            this.targetCamera.updateTarget(this.cameraTarget.target);
        }
    }
    ...
    render(deltaTime) {
        ...
        this.starField.draw(this.ctx, this.camera, this.playerShip.velocity);
        if (this.cameraTarget.starSystem.asteroidBelt) {
            this.cameraTarget.starSystem.asteroidBelt.draw(this.ctx, this.camera);
        }
        this.cameraTarget.starSystem.celestialBodies.forEach(body => body.draw(this.ctx, this.camera));
        this.cameraTarget.starSystem.ships.forEach(ship => ship.draw(this.ctx, this.camera));
    ...
    renderTargetView() {
        let target = this.cameraTarget instanceof Ship ? this.cameraTarget.target : null;
        ...
        if (this.cameraTarget.starSystem.asteroidBelt) {
            this.cameraTarget.starSystem.asteroidBelt.draw(this.targetCtx, this.targetCamera);
        }
        this.cameraTarget.starSystem.celestialBodies.forEach(body => body.draw(this.targetCtx, this.targetCamera));
        this.cameraTarget.starSystem.ships.forEach(ship => ship.draw(this.targetCtx, this.targetCamera));
    }
}
