class GameObject {
    constructor(position, starSystem) {
        ...
        this.starSystem = starSystem; // Moved here from subclasses
...
class CelestialBody extends GameObject {
    constructor(distance, radius, color, parent = null, angle = 0, type = celestialTypes['planet'], subtype = null, name = '', starSystem = null) {
...
class StarSystem {
    constructor(id, name, position, celestialBodies) {
        ...
        this.celestialBodies = celestialBodies.map(body => {
            body.starSystem = this; // Set starSystem for each celestial body
            return body;
        });
        this.ships = [];
        this.maxAIShips = 10;
...
class Ship extends GameObject {
    ...
    initiateHyperjump(targetSystem, currentTime) {
        ...
        this.targetAngle = 0;
        this.trail.points = [];
        this.clearTarget(); // Clear the target on jump
...
class Game {
    ...
    update(deltaTime) {
        ...
        // Check if cameraTarget.target is valid
        if (this.manager.cameraTarget instanceof Ship && this.manager.cameraTarget.target) {
            ...
            if (target.isDespawned() || this.manager.cameraTarget.starSystem !== target.starSystem) {
                ...
                // Show target window if hidden and target is valid
    ...
    renderTargetView() {
        ...
        if (!target || !this.manager.targetingSystem.isValidTarget(target)) { // Updated to use TargetingSystem
...
class GameManager {
    constructor() {
        ...
        this.hud = new HeadsUpDisplay(this, window.innerWidth, window.innerHeight); // Changed from this.game to this
        this.zoomTextTimer = 0;
    ...
    setupEventListeners() {
        ...
        window.addEventListener('keydown', (e) => {
            ...
            if (e.key === 't' || e.key === 'T') { // Check for both 't' and 'T'
                e.preventDefault();
                if (e.shiftKey) {
                    this.targetingSystem.cyclePreviousTarget(this.playerShip);
                } else {
                    this.targetingSystem.cycleNextTarget(this.playerShip);
                }
            }
        });
