...
export class PlayerPilot extends Pilot {
     ...
     * @param {number} deltaTime - Time elapsed since the last update in seconds.
     * @param {GameManager} game - The game manager object (unused in this implementation).
     */
    update(deltaTime, game) {
        ...
        this.ship.applyThrust(game.keys.ArrowUp);
        this.ship.applyBrakes(game.keys.ArrowDown);
        ...
        if (game.keys['t'] && !game.lastKeys['t']) { // 't' pressed this frame
            const targets = this.listTargetableObjects(game);
            if (targets.length > 0) {
        ...
        if (game.keys['T'] && !game.lastKeys['T']) { // 'Shift + T' pressed this frame
            const targets = this.listTargetableObjects(game);
            if (targets.length > 0) {
                const currentIndex = targets.indexOf(this.ship.target);
    ...
    update(deltaTime, manager) {
        ...
        if (this.ship.state === 'Landed') {
            ...
            if (this.landWaitTime <= 0) {
                // Select new destination and set angle before takeoff
                ...
                this.ship.setTargetAngle(desiredAngle); // Set desired angle for takeoff
                this.ship.initiateTakeoff();
                this.landWaitTime = 0;
            }
