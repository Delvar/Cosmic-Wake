...
export class Ship extends GameObject {
    ...
    initiateHyperjump(currentTime) {
        ...
        this.starSystem = gate.lane.target;
        this.position = gate.lane.targetGate.position.clone(); // Use targetGate from Hyperlane
        this.velocity = new Vector2D(0, 0);
        this.angle = 0;
        this.targetAngle = 0;
        this.trail.points = [];
    ...
    */
    draw(ctx, camera) {
        ...
        ctx.beginPath();
        ctx.moveTo(15, 0); // Adjusted for scale applied above
        ctx.lineTo(-10, 10);
        ctx.lineTo(-10, -10);
        ctx.closePath();
        ctx.fill();
