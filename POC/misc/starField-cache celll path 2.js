...
export class StarField {
    ...
    draw(ctx, camera) {
        ...
        const zoomKey = Math.round((quantizedZoom - 0.5) * 10); // Map zoom 0.5-1.0 to 0-5
        ...
        for (let layer = 0; layer < this.layers; layer++) {
            ...
            for (let i = gridLeft; i < gridRight; i++) {
                for (let j = gridTop; j < gridBottom; j++) {
                    // Integer cache key with zoom
                    const cacheKey = ((i % 1000 + 1000) % 1000) * 1e4 +
                                    ((j % 1000 + 1000) % 1000) * 1e1 +
                                    layer +
                                    (zoomKey * 1e7); // Zoom in high bits to avoid collisions
                    ...
                    // Set transform and render paths
                    ctx.setTransform(1, 0, 0, 1, this._scratchScreenCellPos.x, this._scratchScreenCellPos.y);
            ...
            // Reset transform after each layer
