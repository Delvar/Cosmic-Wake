...
export class CargoCollectorAutopilot extends Autopilot {
    ...
    updateCollecting(deltaTime, gameManager) {
        ...
        // If no sub-autopilot or target invalid/despawned, create new one
        if (!this.subAutopilot || this.subAutopilot.completed || this.subAutopilot.target.amount <= 0) {
            if (this.subAutopilot) {
                this.subAutopilot.stop();
            }
            this.subAutopilot = new FlyToTargetAutopilot(this.ship, closest, closest.radius, 50); // Close arrival for pickup
